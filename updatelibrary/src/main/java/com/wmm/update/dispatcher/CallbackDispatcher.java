package com.wmm.update.dispatcher;
...
import com.wmm.update.callback.StringCallback;
import com.wmm.update.exception.RequestException;
...
public class CallbackDispatcher {
    ...
    public static void dispatchRequestStart(final BaseCallback callback) {
        ...
        mDispatcher.post(() -> callback.checkUpdateStart());
    }
    ...
    public static void dispatchRequestSuccess(final StringCallback callback, Response response) throws IOException {
        ...
        if (body != null) {
            ...
            mDispatcher.post(() -> callback.checkUpdateSuccess(result));
        } else {
            mDispatcher.post(() -> callback.checkUpdateFailure(new RequestException("body==null")));
        }
    }
    ...
    public static void dispatchRequestFailure(final BaseCallback callback, final Throwable e) {
        if (callback == null) return;
        mDispatcher.post(() -> callback.checkUpdateFailure(e));
    }
    ...
    public static void dispatchDownloading(final DownloadCallback callback,
        ...
        if (callback == null) return;
        mDispatcher.post(() -> callback.downloadProgress(currentLength, totalLength));
    }
    ...
    public static void dispatchDownloadSuccess(final DownloadCallback callback, final File apk) {
        if (callback == null) return;
        mDispatcher.post(() -> callback.downloadSuccess(apk));
    }
    ...
    public static void dispatchRequestFinish(final BaseCallback callback) {
        if (callback == null) return;
        mDispatcher.post(() -> callback.checkUpdateFinish());
    }
